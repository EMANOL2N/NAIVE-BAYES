...
# Función para evaluar el modelo y calcular la precisión, sensibilidad y especificidad
evaluar_modelo <- function(testData, P_sospechoso, P_no_sospechoso, positivo, negativo, variables) {
  ...
  TN <- tabla_contingencia[1,1]  # Verdaderos negativos
  FP <- tabla_contingencia[2,1]  # Falsos positivos
  FN <- tabla_contingencia[1,2]  # Falsos negativos
  TP <- tabla_contingencia[2,2]  # Verdaderos positivos
  ...
  print(paste("Sensibilidad: ", round(sensibilidad * 100, 2), "%", sep = ""))
  
  # Calcular la especificidad
  especificidad <- TN / (TN + FP)
  print(paste("Especificidad: ", round(especificidad * 100, 2), "%", sep = ""))
  
  # Guardar la matriz de confusión como imagen
  confusion_matrix <- confusionMatrix(predicciones, actuales)
  png("matriz_confusion.png")
  fourfoldplot(confusion_matrix$table, color = c("#CC6666", "#99CC99"), 
               conf.level = 0, margin = 1, main = "Matriz de Confusión Naive Bayes")
  dev.off()
}
...
valores_personalizados2 <- data.frame(
  tos = as.factor(1),  # Tiene tos
  cefalea = as.factor(1),  # No tiene cefalea
  congestion_nasal = as.factor(1),  # Tiene congestión nasal
  dificultad_respiratoria = as.factor(1),  # Tiene dificultad respiratoria
  dolor_garganta = as.factor(1),  # No tiene dolor de garganta
  fiebre = as.factor(1),  # Tiene fiebre
  diarrea = as.factor(1),  # No tiene diarrea
  dolor_abdominal = as.factor(1),  # No tiene dolor abdominal
  dolor_articulaciones = as.factor(1),  # No tiene dolor en articulaciones
  dolor_muscular = as.factor(1)  # Tiene dolor muscular
)

hacer_prediccion_personalizada(valores_personalizados2, P_sospechoso, P_no_sospechoso, positivo, negativo, variables)

valores_personalizados3 <- data.frame(
  tos = as.factor(0),  # Tiene tos
  cefalea = as.factor(0),  # No tiene cefalea
  congestion_nasal = as.factor(0),  # Tiene congestión nasal
  dificultad_respiratoria = as.factor(1),  # Tiene dificultad respiratoria
  dolor_garganta = as.factor(1),  # No tiene dolor de garganta
  ...
  dolor_abdominal = as.factor(0),  # No tiene dolor abdominal
  dolor_articulaciones = as.factor(0),  # No tiene dolor en articulaciones
  dolor_muscular = as.factor(0)  # Tiene dolor muscular
)

hacer_prediccion_personalizada(valores_personalizados3, P_sospechoso, P_no_sospechoso, positivo, negativo, variables)
